...
func _run_turn() -> void:
	...
	game.cue_stick.spin_offset = Vector2.ZERO
	...
	await game.cue_stick.animate_ai_stroke(plan["power"])
	_apply_shot(plan)
	game.ai_turn_active = false
	game._sync_camera_input_with_cue_state()
...
func _plan_shot() -> Dictionary:
	if game.cue_ball == null or not game.can_shoot():
		return {}
	_ensure_shadow()
	var cue_number: int = game.cue_ball.ball_number
	var ball_snapshot: Array = _snapshot_balls()
	var context: Dictionary = _build_context(game.cue_ball.global_position, ball_snapshot, cue_number)
	_pending_plan = {}
...
func _run_cue_placement_task(candidates: Array, ball_snapshot: Array, cue_number: int) -> void:
	...
	for candidate in candidates:
		var score: float = AI_OPPONENT.best_score(_build_context(candidate, ball_snapshot, cue_number))
		if score > best_score:
			best_score = score
...
func _build_context(cue_position: Vector3, ball_snapshot: Array, cue_number: int) -> Dictionary:
	...
	return {
		...
		"shot_outcome": _shot_outcome.bind(ball_snapshot, cue_number),
...
func _refine_bank_direction(choice: Dictionary, power: float, ball_snapshots: Array, cue_number: int) -> Vector3:
	...
	for _attempt in AI_BANK_REFINE_ITERATIONS:
		...
		var contact: Variant = _simulate_strike(ball_snapshots, cue_number, candidate_direction, power)
...
func _simulate_strike(ball_snapshots: Array, cue_number: int, direction: Vector3, power: float) -> Variant:
	...
	_shadow.strike_ball(cue_number, impulse, Vector3.ZERO)
	var steps := 0
...
func _simulate_outcome(ball_snapshots: Array, cue_number: int, direction: Vector3, power: float) -> Dictionary:
	_ensure_shadow()
	_shadow.unregister_all()
	for entry in ball_snapshots:
		_shadow.add_ball(entry["number"], entry["position"], Vector3.ZERO, BALL_RADIUS, BilliardsBall.MASS, Vector3.ZERO)
...
## WorkerThreadPool shot-planning task, where live Node position reads are
## not allowed).
func _shot_outcome(direction: Vector3, power: float, _hint: Dictionary, ball_snapshots: Array, cue_number: int) -> Dictionary:
	if ball_snapshots.is_empty():
		return {"scratched": true}
	var settled := _simulate_outcome(ball_snapshots, cue_number, direction, power)
	...
	return {
		...
		"own_leave_score": AI_OPPONENT.best_score(_shape_context(settled["cue_position"], settled["balls"], GameManager.rules.legal_targets(remaining_numbers))),
		"opponent_score": AI_OPPONENT.best_score(_shape_context(settled["cue_position"], settled["balls"], _opponent_legal_numbers(remaining_numbers))),
	}
...
func _opponent_legal_numbers(remaining: Array[int]) -> Array[int]:
	...
	result.assign(rules.legal_targets(remaining))
